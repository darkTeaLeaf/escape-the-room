...
doorColorToColor :: DoorColor -> CodeWorld.Color
doorColorToColor dc =
  case dc of
    Red         -> CodeWorld.red
    Blue        -> CodeWorld.blue
...
type TupleCoords = (Integer, Integer)
...
level11 = Level "Muhammad Mavlyutov"
  ...
  where
    ...
    levelMap1 (x, y)
      ...
      | otherwise = Floor
     where
      checkRadius :: (Integer, Integer) -> Integer -> Bool
...
level12 = Level "Arina Fedorovskaya"
  ...
  where
    ...
    -- | Traps
    level (Coords 6 (-7)) = Trap Down
    level (Coords 8 (-7)) = Trap Left_
    level (Coords (-4) (-7)) = Trap Right_
    level (Coords 7 (-2)) = Trap Left_
    level (Coords 7 4) = Trap Up
    level (Coords 1 8) = Trap Right_
    level (Coords (-5) 8) = Trap Right_
    level (Coords (-2) 1) = Trap Right_
    level (Coords (-8) 1) = Trap Right_
    -- | Exit
    level (Coords x y)
...
xLineOf :: Integer -> TupleCoords -> Tile -> MapOperation
...
yLineOf :: Integer -> TupleCoords -> Tile -> MapOperation
